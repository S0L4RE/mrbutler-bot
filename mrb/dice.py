...
class Dice(object):
    """
    Simple class, containing static operations to roll dice!
    """
    ...
    def roll(input_string: str='') -> List[int]:
        """
        Given an input string, parse out and perform a "roll" of dice.

        For example, inputting "2d20" would roll TWO dice with TWENTY sides
        each.

        :raises: `ValueError` on parse failure. Contains a readable message.
        ...
        # If any of these fail to parse out, bail out
        if not all(Dice.poor_mans_try_parse(x) for x in input_split):
            raise ValueError(invalid_format_msg)
        ...
        # So we have valid input, within logical bounds. Roll em!
        return Dice.roll_dice(count=number_of_dice, sides=sides_per_die)
